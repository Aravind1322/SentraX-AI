...
def render_enterprise():
    ...
    # ================= API MODULE =================
        ...
        uploaded_file = st.file_uploader(
            ...
            type=["csv"],
            help="CSV must contain columns: amount, location, device"
        )
        ...
        if uploaded_file is not None:
            ...
            required = {"amount", "location", "device"}
            missing = required - set(col_map.keys())

            if missing:
                st.markdown(
                    f'<div class="reason-chip" style="border-left: 3px solid #ff3b30;">'
                    f'⚠️ Missing required columns: <b>{", ".join(sorted(missing))}</b>. '
                    f'Please check your CSV and re-upload.</div>',
                    unsafe_allow_html=True
                )
            else:
                # Remap to canonical lowercase column names for processing
                ...
                df_work["Risk Score"] = df_work.apply(_score_row, axis=1)
                df_work["Status"] = df_work["Risk Score"].apply(get_status)
                ...
                # Merge Risk Score, Status, and Scan Time back onto original df for display
                result_df = raw_df.copy()
                result_df["Risk Score"] = df_work["Risk Score"].values
                result_df["Status"]     = df_work["Status"].values
                result_df["Scan Time"]  = pd.Timestamp.now()
                ...
                st.download_button(
                    ...
                    file_name="enterprise_scan_results.csv",
                    mime="text/csv",
                )
